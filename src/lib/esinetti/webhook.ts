/**
 * eSinetin webhookien vastaanotto (CLAUDE.md 5.4: `round.completed` →
 * sinetöidyt PDF:t, `identity_verified_at`, tenancy `active`).
 *
 * ===========================================================================
 ...
 *    tahansa voisi väittää vuokrasuhteen allekirjoitetuksi.
...
/**
 * Tapahtumat, joihin Reilusoppari reagoi.
 *
 * `round.completed` on se, joka vie vuokrasuhteen eteenpäin. Muut ovat
 * näytettävää tilatietoa: kumpi on avannut, kumpi on tunnistautunut.
 */
...
export interface WebhookEvent {
  ...
  createdAt: string;
  roundId: string;
  /** `tenancy:<uuid>:alku` tai `:loppu` — tämä kertoo mihin vuokrasuhteeseen tapahtuma kuuluu. */
