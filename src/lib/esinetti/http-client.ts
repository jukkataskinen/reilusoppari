/**
 * eSinetin REST-API -client (CLAUDE.md kohta 2).
 *
 * ===========================================================================
 * TIETOTURVAKATSELMUS (esinetti CLAUDE.md 0.1)
 *
 * 1. Kuka saa kutsua: vain palvelinkoodi. API-avain on tenantin `reilusoppari`
 *    pääsy kaikkiin sen asiakirjoihin, joten se ei saa päätyä selaimeen.
 *    `assertServerOnly()` kaataa heti, jos moduuli ladataan selaimessa —
 *    ympäristömuuttujassa ei ole `NEXT_PUBLIC_`-etuliitettä, joten näin ei
 *    pitäisi käydä, mutta virhe on parempi kuin hiljainen `undefined`.
 ...
 *    sitä saa lisätä (CLAUDE.md kohta 6).
    ...
    const payload = await this.request<RoundWire>("POST", "/rounds", {
      body: {
        ...
        signers: input.signers.map((s) => ({
          ...
          // Reilusopparin todistusten arvo perustuu vahvaan tunnistukseen,
          // joten oletus on `strong` eikä eSinetin oletus jää arvattavaksi.
