...
/**
 * Loppukatselmuksen pöytäkirja PDF:nä (CLAUDE.md 5.8).
 *
 * ===========================================================================
...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  let data;
  try {
    data = await buildInspectionProtocolData(user.id, id, "final");
  } catch (err) {
    console.error(
      "[loppukatselmus] pöytäkirjan kokoaminen epäonnistui:",
  ...
  try {
    ...
    return new NextResponse(Buffer.from(result.bytes), {
      headers: {
        ...
        "content-disposition": 'inline; filename="loppukatselmus.pdf"',
    ...
    console.error(
      "[loppukatselmus] renderöinti epäonnistui:",
      err instanceof Error ? err.message : err,
    );
    return new NextResponse("Pöytäkirjaa ei voitu tuottaa.", { status: 500 });
  }
