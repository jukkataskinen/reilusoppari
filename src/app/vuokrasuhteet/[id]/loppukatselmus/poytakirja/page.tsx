...
/**
 * Loppukatselmuksen pöytäkirjan esikatselu (CLAUDE.md 5.8).
 *
...
export default async function ProtocolPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const overview = await getInspectionOverview(user.id, id, "final");
  const pdf = `/vuokrasuhteet/${id}/loppukatselmus/poytakirja/pdf`;

  if (overview.inspection.status === "open") {
    return (
      <AppShell>
        <h1 className="text-2xl">Loppukatselmuksen pöytäkirja</h1>
        <p className="mt-3 text-ink/70">
          Pöytäkirja syntyy, kun loppukatselmus lukitaan. Siihen asti kuvat ovat luonnos, jota voi
          vielä täydentää.
        </p>
  ...
  return (
    <AppShell>
      ...
      <p className="mt-2 text-ink/70">
        Tämä on sama asiakirja, joka allekirjoitetaan. Sen jälkeen kumpikin antaa toisestaan arvion.
      </p>
