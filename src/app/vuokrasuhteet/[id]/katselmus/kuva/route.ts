...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  let inspection;
  try {
    inspection = await getOrCreateInspection(user.id, tenancyId);
  } catch {
  ...
  try {
    ...
    console.error("[katselmus] metatietojen poisto epäonnistui");
  ...
  */
  ...
  if (uploadError) {
    console.error("[katselmus] kuvan tallennus epäonnistui:", uploadError.message);
