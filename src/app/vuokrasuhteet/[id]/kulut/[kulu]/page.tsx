...
/**
 * Yksi kulu ja sen kuitit (CLAUDE.md 5.7).
 *
 * Sivu on vain asunnon omistajalle: `getExpense` tekee omistajatarkistuksen
 * ja heittää muille. Vuokralainen saa saman 404:n kuin ulkopuolinen, koska
 * hänen ei kuulu tietää kulun olemassaolostakaan.
 */
export default async function ExpensePage({
  ...
  let expense;
  try {
    expense = await getExpense(user.id, id, kulu);
  } catch {
    notFound();
  }
  if (!expense) notFound();
  ...
  return (
    <AppShell>
      ...
      <PhotoCapture
        endpoint={`/vuokrasuhteet/${id}/kulut/${kulu}/kuitti`}
        label="Kuvaa kuitti"
      />
      ...
      <p className="mt-10 text-sm">
        <Link href={`/vuokrasuhteet/${id}/kulut`} className="underline underline-offset-4">
          {fi.common.back}
        </Link>
      </p>
    </AppShell>
